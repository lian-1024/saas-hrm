...
const GivePermissionModal = defineAsyncComponent(
  () => import('../components/give-permission-modal.vue'),
)
// 角色管理
defineOptions({
  name: 'RolePage',
...
<template>
  <Flex class="role-wrapper" vertical align="start" gap="middle">
    <Button type="primary" @click="addRoleModalStatus = true">{{
      $t('role.table.actions.addRole')
    }}</Button>
    <QSpin :spinning="getRoleListLoading || updateRoleLoading" wrapper-class-name="flex-1 w-full">
      <Table
        ...
        :columns="roleColumns"
        :dataSource="roleTableDataSource.rows"
        bordered
      >
        ...
        <template #bodyCell="{ column, record, text }">
          ...
          <template v-if="canEditable.includes(String(column.dataIndex))">
            ...
            <template v-if="editableData[record.id]">
              ...
              <Switch
                ...
                :checked="roleIsEnable(record.id)"
          ...
          <template v-if="column.key === 'actions'">
            <Flex gap="small" wrap="wrap">
              <template v-if="!editableData[record.id]">
                <TypographyLink @click="handleGivePermission(record.id)"
                  >{{ $t('role.table.actions.givePermission') }}
                </TypographyLink>
                <TypographyLink @click="handleEditRole(record.id)">{{
                  $t('role.table.actions.edit')
                }}</TypographyLink>
                <Popconfirm
                  :title="t('role.table.operationMessage.deleteConfirmTitle')"
              ...
              <template v-else>
                <TypographyLink @click="handleSaveEditRole(record.id)"
                  >{{ $t('role.table.actions.save') }}
                </TypographyLink>
                <Popconfirm
                  :title="$t('role.table.operationMessage.cancelConfirmTitle')"
