...
import type { ModalTypeKey } from '../constants'
...
const handleSubmit = () => {
  formRef.value?.validate().then(async () => {
    ...
    if (!props.departmentId)
      return message.error(t('department.operations.operationMessage.selectDepartment'))
...
const loading = computed(
  () =>
    getDepartmentDetailLoading.value ||
    getDepartmentManagerListLoading.value ||
    updateDepartmentLoading.value ||
    addLoading.value,
)
</script>
      ...
      <div class="department-modal">
        <Form ref="formRef" layout="vertical" :model="formData" :rules="formRules">
          ...
          <Form.Item :label="t('department.form.fields.managerId')" name="managerId">
            <Select
              :placeholder="t('department.form.placeholder.managerId')"
          ...
          <Form.Item :label="t('department.form.fields.introduce')" name="introduce">
            <Input.TextArea
              ...
              :placeholder="t('department.form.placeholder.introduce')"
              :maxLength="100"
